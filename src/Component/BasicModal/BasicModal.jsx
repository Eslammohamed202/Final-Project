...
import { cash, onlinePayement } from "../../APIS/payment";
...
export default function BasicModal({cartId}) {
  ...
  let {mutate, data} = useMutationCart(onlinePayement)
  ...
  return (
    <div>
      <Button color="success" variant="contained" sx={{my:'30px'}} onClick={()=>{handleOpen();setFlag(true)}}>Pay Online</Button>
      <Button color="success" variant="contained" sx={{mx:'30px'}} onClick={handleOpen}>Pay Cash</Button>
      <Modal
        open={open}
        onClose={handleClose}
